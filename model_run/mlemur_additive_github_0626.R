...
# Additive model: rainfall + maximum temperature + population density
...
js_model_additive <- nimbleCode( {
  ...
  # priors for beta coefficients of covariate effects on survival (additive, no interactions):
  ...
  for (t in 1:(n.occasions-1)){
    ...
    # monthly survival, additive model:
    ...
    # from NE state:
    ps[1,1:M,t,1] <- nimNumeric(length = M, value = 1-gamma[t])                                                       # NE to NE
    ps[1,1:M,t,2] <- nimNumeric(length = M, value = (equals(t,1)*pi[1] + step(t-2))*gamma[t]*dry_season[t])           # NE to JD (entry only in dry season)
    ps[1,1:M,t,3] <- nimNumeric(length = M, value = 0)                                                                # NE to JW
    ps[1,1:M,t,4] <- nimNumeric(length = M, value = equals(t,1)*pi[2]*gamma[t])                                       # NE to AD (only in first occasion)
    ps[1,1:M,t,5] <- nimNumeric(length = M, value = 0)                                                                # NE to AW
    ps[1,1:M,t,6] <- nimNumeric(length = M, value = 0)                                                                # NE to dead
    
    # from JD state:
    ps[2,1:M,t,1] <- nimNumeric(length = M, value = 0)              # JD to NE
    ps[2,1:M,t,2] <- nimNumeric(length = M, value = 0)              # JD to JD
    ps[2,1:M,t,3] <- nimNumeric(length = M, value = phi_JD[t])      # JD to JW
    ps[2,1:M,t,4] <- nimNumeric(length = M, value = 0)              # JD to AD
    ps[2,1:M,t,5] <- nimNumeric(length = M, value = 0)              # JD to AW
    ps[2,1:M,t,6] <- nimNumeric(length = M, value = 1-phi_JD[t])    # JD to dead
    
    # from JW state:
    ps[3,1:M,t,1] <- nimNumeric(length = M, value = 0)              # JW to NE
    ps[3,1:M,t,2] <- nimNumeric(length = M, value = 0)              # JW to JD
    ps[3,1:M,t,3] <- nimNumeric(length = M, value = 0)              # JW to JW
    ps[3,1:M,t,4] <- nimNumeric(length = M, value = phi_JW[t])      # JW to AD
    ps[3,1:M,t,5] <- nimNumeric(length = M, value = 0)              # JW to AW
    ps[3,1:M,t,6] <- nimNumeric(length = M, value = 1-phi_JW[t])    # JW to dead
    
    # from AD state:
    ps[4,1:M,t,1] <- nimNumeric(length = M, value = 0)              # AD to NE
    ps[4,1:M,t,2] <- nimNumeric(length = M, value = 0)              # AD to JD
    ps[4,1:M,t,3] <- nimNumeric(length = M, value = 0)              # AD to JW
    ps[4,1:M,t,4] <- nimNumeric(length = M, value = 0)              # AD to AD
    ps[4,1:M,t,5] <- nimNumeric(length = M, value = phi_AD[t])      # AD to AW
    ps[4,1:M,t,6] <- nimNumeric(length = M, value = 1-phi_AD[t])    # AD to dead
    
    # from AW state:
    ps[5,1:M,t,1] <- nimNumeric(length = M, value = 0)              # AW to NE
    ps[5,1:M,t,2] <- nimNumeric(length = M, value = 0)              # AW to JD
    ps[5,1:M,t,3] <- nimNumeric(length = M, value = 0)              # AW to JW
    ps[5,1:M,t,4] <- nimNumeric(length = M, value = phi_AW[t])      # AW to AD
    ps[5,1:M,t,5] <- nimNumeric(length = M, value = 0)              # AW to AW
    ps[5,1:M,t,6] <- nimNumeric(length = M, value = 1-phi_AW[t])    # AW to dead
    
    # from dead state:
    ps[6,1:M,t,1] <- nimNumeric(length = M, value = 0)              # dead to NE
    ps[6,1:M,t,2] <- nimNumeric(length = M, value = 0)              # dead to JD
    ps[6,1:M,t,3] <- nimNumeric(length = M, value = 0)              # dead to JW
    ps[6,1:M,t,4] <- nimNumeric(length = M, value = 0)              # dead to AD
    ps[6,1:M,t,5] <- nimNumeric(length = M, value = 0)              # dead to AW
    ps[6,1:M,t,6] <- nimNumeric(length = M, value = 1)              # dead to dead
    
    
    ## Define probabilities of O(t) given S(t) ##
    
    po[1,1:M,t,1] <- nimNumeric(length = M, value = 0)              # NE: not seen as JD
    po[1,1:M,t,2] <- nimNumeric(length = M, value = 0)              # NE: not seen as JW
    po[1,1:M,t,3] <- nimNumeric(length = M, value = 0)              # NE: not seen as AD
    po[1,1:M,t,4] <- nimNumeric(length = M, value = 0)              # NE: not seen as AW
    po[1,1:M,t,5] <- nimNumeric(length = M, value = 1)              # NE: not seen
    
    po[2,1:M,t,1] <- nimNumeric(length = M, value = p_JD[t])        # JD: seen as JD
    po[2,1:M,t,2] <- nimNumeric(length = M, value = 0)              # JD: not seen as JW
    po[2,1:M,t,3] <- nimNumeric(length = M, value = 0)              # JD: not seen as AD
    po[2,1:M,t,4] <- nimNumeric(length = M, value = 0)              # JD: not seen as AW
    po[2,1:M,t,5] <- nimNumeric(length = M, value = 1-p_JD[t])      # JD: not seen
    
    po[3,1:M,t,1] <- nimNumeric(length = M, value = 0)              # JW: not seen as JD
    po[3,1:M,t,2] <- nimNumeric(length = M, value = p_JW[t])        # JW: seen as JW
    po[3,1:M,t,3] <- nimNumeric(length = M, value = 0)              # JW: not seen as AD
    po[3,1:M,t,4] <- nimNumeric(length = M, value = 0)              # JW: not seen as AW
    po[3,1:M,t,5] <- nimNumeric(length = M, value = 1-p_JW[t])      # JW: not seen
    
    po[4,1:M,t,1] <- nimNumeric(length = M, value = 0)              # AD: not seen as JD
    po[4,1:M,t,2] <- nimNumeric(length = M, value = 0)              # AD: not seen as JW
    po[4,1:M,t,3] <- nimNumeric(length = M, value = p_AD[t])        # AD: seen as AD
    po[4,1:M,t,4] <- nimNumeric(length = M, value = 0)              # AD: not seen as AW
    po[4,1:M,t,5] <- nimNumeric(length = M, value = 1-p_AD[t])      # AD: not seen
    
    po[5,1:M,t,1] <- nimNumeric(length = M, value = 0)              # AW: not seen as JD
    po[5,1:M,t,2] <- nimNumeric(length = M, value = 0)              # AW: not seen as JW
    po[5,1:M,t,3] <- nimNumeric(length = M, value = 0)              # AW: not seen as AD
    po[5,1:M,t,4] <- nimNumeric(length = M, value = p_AW[t])        # AW: seen as AW
    po[5,1:M,t,5] <- nimNumeric(length = M, value = 1-p_AW[t])      # AW: not seen
    
    po[6,1:M,t,1] <- nimNumeric(length = M, value = 0)              # dead: not seen as JD
    po[6,1:M,t,2] <- nimNumeric(length = M, value = 0)              # dead: not seen as JW
    po[6,1:M,t,3] <- nimNumeric(length = M, value = 0)              # dead: not seen as AD
    po[6,1:M,t,4] <- nimNumeric(length = M, value = 0)              # dead: not seen as AW
    po[6,1:M,t,5] <- nimNumeric(length = M, value = 1)              # dead: not seen
  ...
  z[1:M,1] <- 1  # all M individuals are in state NE at t=1
  
  for (t in 2:n.occasions){
    for (i in 1:M){
      z[i,t] ~ dcat(ps[z[i,t-1], i, t-1, 1:6])  # state process
      y[i,t] ~ dcat(po[z[i,t], i, t-1, 1:5])     # observation process
    } # i
  } # t
...
# Added dummy occasion to initial values (1993_2 is the dummy occasion, state = NE = 1):
my.z.init <- cbind(rep(1, dim(samperCH_inits)[1]), samperCH_inits)
colnames(my.z.init)[1] <- "1993_2"

nz <- dim(samperCH_ms)[1]  # augmented individuals (equal to observed)
my.z.init.ms <- rbind(my.z.init, matrix(1, ncol = dim(my.z.init)[2], nrow = nz))
str(my.z.init.ms)
table(my.z.init.ms)


# Add dummy occasion to the data (column of "not seen" = 5 before first occasion):
...
# augment CH.ms with nz rows of all-not-seen (5) for the unobserved individuals:
CH.ms <- rbind(CH.du, matrix(5L, nrow = nz, ncol = dim(CH.du)[2]))

# dry season indicator (1 = dry/postwet, 0 = wet/prewet):
dry_season <- (1:(dim(CH.ms)[2]-1)) %% 2
length(dry_season)

# Dirichlet alpha for starting proportions of JD and AD in year 1:
...
js_add.model <- nimbleModel(code      = js_model_additive,
                            constants = nimble_constants,
                            data      = nimble_data,
                            inits     = inits())

C.js_add.model <- compileNimble(js_add.model)
js_add.config  <- configureMCMC(js_add.model, monitors = params)
js_add.MCMC    <- buildMCMC(conf = js_add.config, useConjugacy = FALSE)
C.js_add.MCMC  <- compileNimble(js_add.MCMC, project = js_add.model)
...
run_js_additive <- runMCMC(mcmc             = C.js_add.MCMC,
                           nchains          = nchains,
                           nburnin          = nburnin,
                           niter            = niter,
                           thin             = thin,
