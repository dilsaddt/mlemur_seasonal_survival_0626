...
# Interaction model I: rainfall x population density
...
js_model_interaction1 <- nimbleCode( {
  ...
  # interaction terms: rainfall x population density
  beta.JD.rain_prev_dens_current    ~ dnorm(mean = 0, sd = 2)
  beta.AD.rain_prev_dens_current    ~ dnorm(mean = 0, sd = 2)
  beta.JW.rain_current_dens_current ~ dnorm(mean = 0, sd = 2)
  beta.AW.rain_current_dens_current ~ dnorm(mean = 0, sd = 2)
  ...
  for (t in 1:(n.occasions-1)){
    ...
    # monthly survival, interaction model I (rain x density):
...
inits <- function(){list(
  ...
  beta.JD.rain_prev_dens_current    = rnorm(1, 0, 1),
  beta.AD.rain_prev_dens_current    = rnorm(1, 0, 1),
  beta.JW.rain_current_dens_current = rnorm(1, 0, 1),
  beta.AW.rain_current_dens_current = rnorm(1, 0, 1),
...
params <- c(
  ...
  "beta.JD.rain_prev_dens_current",    "beta.AD.rain_prev_dens_current",
  "beta.JW.rain_current_dens_current", "beta.AW.rain_current_dens_current",
  "gamma", "pi",
...
js_int1.model <- nimbleModel(code      = js_model_interaction1,
                             constants = nimble_constants,
                             data      = nimble_data,
                             inits     = inits(),
                             calculate = FALSE)

C.js_int1.model <- compileNimble(js_int1.model)
js_int1.config  <- configureMCMC(js_int1.model, monitors = params)
js_int1.MCMC    <- buildMCMC(conf = js_int1.config, useConjugacy = FALSE)
C.js_int1.MCMC  <- compileNimble(js_int1.MCMC, project = js_int1.model)
...
run_js_interaction1 <- runMCMC(mcmc             = C.js_int1.MCMC,
                               nchains          = nchains,
                               nburnin          = nburnin,
                               niter            = niter,
                               thin             = thin,
...
output_name   <- "js_model_interaction1_rain_dens"
timestamp.now <- gsub(x = gsub(x = as.character(now()), pattern = ":", replacement = ""), pattern = " ", replacement = "-")
nz <- dim(samperCH_ms)[1]
save(runTime.Compile, runTime.Sample, seed, run_js_interaction1,
